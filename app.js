...
const firebaseConfig = {
  ...
  measurementId: "G-FV4GMT8EP2"
    ...
    const showToast = (message, type = 'info', duration = 3000) = {
        ...
        toast.innerHTML = `div class=toast-headerstrong${typeTitle[type]}strongbutton type=button class=toast-close&times;buttondivdiv${message}div`;
    ...
    function populateAllSelects() {
        ...
        $$('.member-select').forEach(sel = sel.innerHTML = `option value=請選擇會員option${memberOptions}`);
        $('#takeMemberSelect').innerHTML += 'option value=non-member非會員option';
        $$('.beer-select').forEach(sel = sel.innerHTML = `option value=請選擇酒款option${beerOptions}`);

        const allBrands = [...new Set(appState.inventory.map(i = i.brand))];
        const brandOptions = allBrands.map(b = `option value=${b}${b}option`).join('');
        $$('.brand-select').forEach(sel = sel.innerHTML = `option value=選擇品牌option${brandOptions}option value=other其他option`);
        
        const allMls = [...new Set(appState.inventory.map(i = i.ml))];
        const mlOptions = allMls.map(m = `option value=${m}${m}mloption`).join('');
        $$('.ml-select').forEach(sel = sel.innerHTML = `option value=選擇ml數option${mlOptions}option value=other其他option`);
    }
            ...
            return `
                div class=brand-card
                    div class=brand-header
                        ...
                        div class=brand-stock合計庫存 ${brandStock} 瓶div
                        div class=brand-expand-icon▼div
                    div
                    div class=brand-water-leveldiv class=water-level-containerdiv class=water-level-fill style=width ${percentage}% title=${percentage.toFixed(1)}%divdivdiv
                    div class=brand-details
                        ${items.map(item = `
                            div class=brand-item
                                span${item.name} ${item.ml}mlspan
                                div class=item-detailsspan class=item-price$${item.price}spanspan庫存 ${item.stock}spandiv
                            div`).join('')}
                    div
                div`;
        }).join('');
